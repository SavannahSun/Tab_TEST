...
# RESULTS_CSV = "tabpfn_openml_results.csv"  # 结果保存文件
RESULTS_CSV = "tabpfn_openml_results_noShuffle.csv"  # 结果保存文件
SKIPPED_CSV = "tabpfn_openml_skipped.csv"  # 被跳过的数据集
...
for dataset_file in dataset_files:
    ...
    # **加载数据**
    X, y, num_classes = get_X_y(file_path, shuffle_data=False)
    ...
    for fold, (train_idx, test_idx) in enumerate(kf.split(X, y), 1):
        ...
        # **初始化 TabPFN**
        clf = TabPFNClassifier(device="cuda:1" if torch.cuda.is_available() else "cpu",
...
print(f"🚨 跳过的数据集已实时保存至 {SKIPPED_CSV}")